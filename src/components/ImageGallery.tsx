...
const ImageGallery = () => {
  ...
  const images = [
    { src: mountainLandscape, alt: '', category: 'nature' },
    { src: professionalPortrait, alt: '', category: 'people' },
    { src: modernBuilding, alt: '', category: 'architecture' },
    { src: abstractPattern, alt: '', category: 'abstract' }
  ];
  ...
  return (
    <div className="stat-card p-4 sm:p-6 rounded-lg mb-6">
      <div className="text-base sm:text-lg font-bold nav-text mb-4">Galeria de Fotos</div>

      <div className="mb-4 sm:mb-6 p-3 sm:p-4 border border-border rounded-lg bg-muted/50">
        <div className="flex items-center gap-2 mb-3">
          <Filter className="h-3 w-3 sm:h-4 sm:w-4 nav-text" />
          <div className="text-sm sm:text-base font-medium nav-text">Filtrar por Categoria</div>
        </div>
            ...
            <CarouselItem key={`${image.src}-${index}`} className="">
              <div className="relative group w-full">
                <img
                  src={image.src}
                  alt=""
                  className="w-full h-56 sm:h-64 lg:h-72 xl:h-80 object-contain bg-muted rounded cursor-pointer hover:opacity-90 transition-opacity"
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity rounded flex items-center justify-center">
                  <div className="flex gap-1 sm:gap-2">
                    <button
                      ...
                      className="p-1 sm:p-2 bg-white/90 text-background rounded-full hover:bg-white transition-colors"
                    >
